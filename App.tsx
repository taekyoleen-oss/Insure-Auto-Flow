...
import { savePipeline, loadPipeline } from "./shared/utils/fileOperations.js";
import {
  loadSampleFromFolder,
  loadFolderSamples,
} from "./shared/utils/samples.js";
...
`;
  ...
  const handleGeneratePPTs = useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        addLog(
          "ERROR",
          `PPT 생성 실패: 서버에 연결할 수 없습니다. 서버가 실행 중인지 확인하세요. (http://localhost:3002)`
        );
  ...
  const loadFolderSamplesLocal = useCallback(async () => {
    ...
    try {
      const samples = await loadFolderSamples(
        "http://localhost:3001/api/samples/list"
      );
  ...
  useEffect(() => {
    const handleWindowMouseMove = (e: globalThis.MouseEvent) => {
      if (isDraggingControlPanel.current) {
        // fixed positioning을 사용하므로 viewport 좌표 사용
        setControlPanelPos({
          x: e.clientX - controlPanelDragOffset.current.x,
          y: e.clientY - controlPanelDragOffset.current.y,
        });
      }
    };
  ...
  const handleControlPanelMouseDown = (e: React.MouseEvent) => {
    ...
    // If it's the first drag (currently centered via CSS), set explicit coordinates
    if (!controlPanelPos) {
      setControlPanelPos({
        x: rect.left,
        y: rect.top,
    ...
    <div className="bg-gray-900 text-white h-screen w-screen flex flex-col overflow-hidden">
      ...
      {viewingDataForModule &&
        (viewingDataForModule.outputData?.type === "DataPreview" ||
          ...
          viewingDataForModule.outputData?.type === "MissingHandlerOutput" ||
          viewingDataForModule.outputData?.type === "EncoderOutput" ||
          viewingDataForModule.outputData?.type === "NormalizerOutput") && (
          <ErrorBoundary>
            <DataPreviewModal
